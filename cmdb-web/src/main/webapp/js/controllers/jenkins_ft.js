...
app.controller('jenkinsFtCtrl', function ($scope, $state, $uibModal, $sce, $timeout, httpService, toaster, staticModel) {
    $scope.authPoint = $state.current.data.authPoint;

    $scope.jobEnvType = staticModel.jenkinsJobEnvType;

    $scope.thisBuildType = 0;
    ...
    $scope.viewBuilds = function (jobItem) {
        ...
        var modalInstance = $uibModal.open({
            templateUrl: 'jenkinsJobBuildsModal',
            controller: 'jenkinsJobBuildsInstanceCtrl',
            size: 'lg',
            resolve: {
                httpService: function () {
                    return httpService;
 ...
 * 任务详情
 */
app.controller('jenkinsJobBuildsInstanceCtrl', function ($scope, $uibModalInstance, $sce, toaster, httpService, jobEnvType, jobItem) {
