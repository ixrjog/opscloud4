...
app.controller('getwayHostConfigCtrl', function ($scope, $state, $uibModal, toaster, httpService, staticModel) {
    ...
    $scope.filegroupName = "filegroup_getway";
    ...
    $scope.addItem = function () {
        var body = {
            id: 0,
            fileName: "getway_host.conf",
            fileDesc: "Getway全局配置文件（系统内置请勿删除）",
            filePath: "/data/www/getway/users/.public",
            fileType: 1,
            useType: 0,
            fileGroupId: 0,
...
app.controller('getwayUserConfigCtrl', function ($scope, $state, $uibModal, toaster, httpService, staticModel) {
    ...
    $scope.launchItem = function (item) {
        var url = "/box/user/getway/launch?username=" + item.username;
...
app.controller('configCopyInstanceCtrl', function ($scope, $uibModalInstance, httpService, item) {
    ...
    // /config/file/getGetwayPath

    $scope.queryGetwayPath = function () {
        var url = "/config/file/getGetwayPath";

        httpService.doGet(url).then(function (data) {
            if (data.success) {
                var body = data.body;
                $scope.item.src = body;
                $scope.item.dest = body;
 ...
 * GETWAY参数配置
 */
app.controller('getwayConfigCtrl', function ($scope, $state, $uibModal, $sce, httpService, toaster) {
