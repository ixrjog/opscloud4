...
import static com.baiyi.opscloud.common.constants.SingleTaskConstants.PULL_ALIYUN_ONS_ROCKETMQ_INSTANCE_TARGET_GROUP;
...
public class AliyunOnsRocketMqInstanceTargetGroupProvider extends AbstractAssetRelationProvider<OnsInstanceInServiceListResponse.InstanceVO, OnsGroupListResponse.SubscribeInfoDo> {
    ...
    @Override
    protected List<OnsInstanceInServiceListResponse.InstanceVO> listEntities(DsInstanceContext dsInstanceContext, OnsGroupListResponse.SubscribeInfoDo target) {
        AliyunConfig.Aliyun aliyun = buildConfig(dsInstanceContext.getDsConfig());
        return aliyunOnsRocketMqInstanceDrive.listInstance(aliyun.getRegionId(), aliyun).stream().filter(e ->
