...
@Slf4j
@Component
public class ZabbixActionDatasource extends BaseZabbixDatasource<ZabbixAction> {
    ...
    public void create(ZabbixConfig.Zabbix zabbix, String actionName, String usergrpName) {
        ZabbixHostGroup zabbixHostGroup = zabbixHostGroupDatasource.getByName(zabbix, ZabbixUtil.toHostgroupName(usergrpName));
        ZabbixFilter filter = ZabbixFilterBuilder.builder()
                .putEntry("evaltype", 1)
    ...
    private List<ZabbixActionParam.Operation> buildOperations(ZabbixConfig.Zabbix zabbix, String usergrpName) {
        ZabbixUserGroup zabbixUserGroup = zabbixUserGroupDatasource.getByName(zabbix, usergrpName);
