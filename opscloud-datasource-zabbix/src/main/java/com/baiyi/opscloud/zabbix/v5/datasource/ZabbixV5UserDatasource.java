...
@Slf4j
@Component
public class ZabbixV5UserDatasource extends AbstractZabbixV5UserDatasource {
    ...
    @CacheEvict(cacheNames = CachingConfiguration.Repositories.ZABBIX, key = "#config.url + '_v5_user_userid_' + #userid")
    public void evictById(ZabbixConfig.Zabbix config, String userid) {
        log.info("清除ZabbixUser缓存 : userid = {}", userid);
    }

    @Cacheable(cacheNames = CachingConfiguration.Repositories.ZABBIX, key = "#config.url + '_v5_user_userid_' + #userid", unless = "#result == null")
