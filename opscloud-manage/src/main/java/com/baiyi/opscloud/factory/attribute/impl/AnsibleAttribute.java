...
@Slf4j
@Component
public class AnsibleAttribute extends BaseAttribute {
    ...
    public Map<String, List<OcServer>> grouping(OcServerGroup ocServerGroup, boolean isSubgroup, int envType) {
        Map<String, List<OcServer>> serverMap = groupingByEnv(ocServerGroup, envType);
        if (isSubgroup)
            groupingSubgroup(serverMap, getSubgroup(ocServerGroup));
        return serverMap;
    }
