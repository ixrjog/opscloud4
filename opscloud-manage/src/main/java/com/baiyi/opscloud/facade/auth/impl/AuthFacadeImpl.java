...
public class AuthFacadeImpl implements AuthFacade {
    ...
    public void deleteRoleById(int id) {
        if (authRoleResourceService.countByRoleId(id) != 0)
            throw new AuthCommonException(ErrorEnum.AUTH_ROLE_HAS_USED);
    ...
    public void deleteGroupById(int id) {
        if (authResourceService.countByGroupId(id) != 0)
            throw new AuthCommonException(ErrorEnum.AUTH_GROUP_HAS_USED);
    ...
    @Override
    @Transactional(rollbackFor = {AuthCommonException.class, Exception.class})
