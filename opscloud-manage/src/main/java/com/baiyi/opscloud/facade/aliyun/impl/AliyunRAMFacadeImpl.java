...
import com.baiyi.opscloud.decorator.aliyun.ram.AliyunRAMDecorator;
...
public class AliyunRAMFacadeImpl implements AliyunRAMFacade {
    ...
    @Override
    public void delRAMUserByUsername(String username) {
        aliyunCore.getAccounts().forEach(e -> {
            OcAliyunRamUser ocAliyunRamUser = ocAliyunRamUserService.queryOcAliyunRamUserByUniqueKey(e.getUid(), username);
            if (ocAliyunRamUser != null && ocAliyunRamUser.getRamType() == 1) {
                if (aliyunRAMUserHandler.deleteRamUser(e, username)){
